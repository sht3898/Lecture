...
def start_simulation(zone, test_second):
    ...
    for second in range(0, test_second):
        if zone.is_new_order():
            input_order = Order(second)
            zone.order_queue.append(input_order)

        for food_truck in zone.food_trucks:
            if(not food_truck.is_busy()) and (len(zone.order_queue) != 0):
                new_order = zone.order_queue.pop(0)
                zone.waiting_times.append(new_order.wait_time(second))
                food_truck.start_next_food(new_order)
                price = food_truck.price
                qty = new_order.get_qty()
                zone.income.append(price * qty)
